...
class EditFragment : Fragment() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
       if(item.itemId==R.id.delete){
            val deleteNote=Model(args.id,args.title,args.groceries,args.medicines,args.others)
            val builder= AlertDialog.Builder(requireContext())
            builder.setPositiveButton("Yes"){
                    _,_->mBuylistViewModel.deleteNote(deleteNote)
            ...
            val updatedNote=binding.groceriesedittext.text.toString()
           val updatedmedicines = binding.medicineseditnote.text.toString()
           val updatedothers = binding.otheritemseditnote.text.toString()
            val update=Model(args.id,args.title,updatedNote,updatedmedicines,updatedothers)
            mBuylistViewModel.viewNote(update)
